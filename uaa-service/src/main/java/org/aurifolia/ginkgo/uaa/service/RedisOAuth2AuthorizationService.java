...
import org.aurifolia.ginkgo.uaa.dto.OAuth2AuthorizationDTO;
...
public class RedisOAuth2AuthorizationService implements OAuth2AuthorizationService {
    private static final String OAUTH2_AUTHORIZATION_KEY_PATTERN = "oauth2:auth:%s";
    private static final String OAUTH2_AUTHORIZATION_INDEX_KEY_PATTERN = "oauth2:%s:%s";
    ...
    public void save(OAuth2Authorization authorization) {
        Map<String, Long> oAuth2AuthorizationIndexes = getOAuth2AuthorizationIndexes(authorization);
        ...
        redisTemplate.opsForValue().set(getOAuth2AuthorizationKey(authorization),
                OAuth2AuthorizationDTO.from(authorization), maxTtlMs, TimeUnit.MILLISECONDS);
    }

    @Override
    public void remove(OAuth2Authorization authorization) {
        redisTemplate.delete(getOAuth2AuthorizationKey(authorization));
        getOAuth2AuthorizationIndexes(authorization).keySet().forEach(redisTemplate::delete);
    }
    ...
    public OAuth2Authorization findByToken(String token, OAuth2TokenType tokenType) {
        String id = (String) redisTemplate.opsForValue()
                .get(getOAuth2AuthorizationIndexKey(tokenType.getValue(), token));
        if (id == null) {
            return null;
        }
        OAuth2AuthorizationDTO oAuth2AuthorizationDTO = (OAuth2AuthorizationDTO) redisTemplate
                .opsForValue().get(getOAuth2AuthorizationKey(id));
        if (oAuth2AuthorizationDTO == null) {
            return null;
        }
        return oAuth2AuthorizationDTO.toOAuth2Authorization();
    }
    ...
    private String getOAuth2AuthorizationIndexKey(String tokenType, String token) {
        return String.format(OAUTH2_AUTHORIZATION_INDEX_KEY_PATTERN, token, tokenType);
    }
